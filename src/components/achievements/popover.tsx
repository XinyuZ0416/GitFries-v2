...
export default function AchievementPopover() {
  ...
  const [ isIssueHoarderBadgeShown, setIsIssueHoarderBadgeShown ] = useState<boolean>(false);
  ...
  const handleCloseIssueHoarderBadge = async() => {
    await updateDoc(doc(db, "users", uid), { hasSeenIssueHoarderBadge: true });
    setIsIssueHoarderBadgeShown(false);
  }
  ...
  useEffect(() => {
    ...
    if (!hasFaved20Issues || hasSeenIssueHoarderBadge == null) {
      setIsIssueHoarderBadgeShown(true);
    }
  ...
  return (
    ...
    <div className={`${isIssueHoarderBadgeShown ? 'flex' : 'hidden'} fixed top-0 start-0 z-50 bg-slate-300/50 w-full h-full items-center justify-center`}>
      <div className="relative bg-white p-6 rounded-lg shadow-md max-w-md w-full">
        <div className="flex justify-end">
          <button onClick={handleCloseIssueHoarderBadge} type="button" className="bg-white rounded-md p-2 text-gray-400 hover:text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500">
            <span className="sr-only">Close menu</span>
            <svg className="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
